...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="english",
        help='Language of the text. Language must be "chinese" or supported by NLTK',
    )
    # New argument
    parser.add_argument(
        "--use-word-tokenizer",
        "-w",
        action="store_true",
        help="Use word tokenizer for the language. Default is false",
    )
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
...
def main():
    print(
        f"This is the M2QA metric evaluation script. It is an adapted version of the official SQuAD v2.0 evaluation script but utilizes the word tokenization instead of the whitespace tokenization that SQuAD uses."
    )
    language = OPTS.language.lower()
    use_word_tokenizer = OPTS.use_word_tokenizer
    ...
    exact_raw, f1_raw = get_raw_scores(dataset, preds, language, use_word_tokenizer)
